...
stringstream get_multiline_message(int socket_fd, char *buffer) {
    ...
    while (!message_ended) {
        ...
        last = read(socket_fd, buffer, 4096);
        if (last == -1) {
...
int main(int argc, char* argv[]) {
    ...
    string tmp;
    list_response_stream >> tmp; // "+OK
    list_response_stream >> tmp; // number of messages
    list_response_stream >> tmp; // "messages"
    int message_id;
    int message_size;

    while (list_response_stream >> message_id) {
        list_response_stream >> message_size;
        cout << "Downloading message #" << message_id << endl;
        ...
        int ch;
        while (retr_response_stream.get() != '\n') {} // ignore first line
        while ((ch = retr_response_stream.get()) != stringstream::traits_type::eof()) {
           file_stream << (char)ch;
        }
